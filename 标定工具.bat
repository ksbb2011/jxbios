...
echo     6. iMouse 探活与实测
echo     7. UI 落点校准      （按钮点得中不中，自带菜单，只测量不改配置）
echo  ----------------------------------------------------------------
echo     0. 退出
echo  ================================================================
echo.
...
echo.
set "UF="
set /p "UF=上浮值(建议 0.3；目标 z_press 5.9~6.0，越大压得越深): "
if "%UF%"=="" set "UF=0.3"
echo.
%PY% tools\calibrate_arm_ink.py --scan-z --z-start 4.8 --z-max 6.2 --z-step 0.1 --up-float %UF%
echo.
pause
goto MENU
...
:DO_TAP_UI
cls
echo  [7] UI 落点校准：验证「期望点能不能触发」
echo      墨点标定覆盖不到屏幕上下边缘，这里改用 App 自身的可见反应当反馈：
echo      先点 App 本来要点的那个点；没反应就按 6pt 步长一圈圈往外试，
echo      首次有反应即停 —— 那个点与期望点的差就是落点偏差。
echo      页面由人工摆好（菜单里会提示每个目标的前置条件）。
echo      结果只**追加**写 data\ui_tap_calib.json，不改任何配置。
echo  ------------------------------------------------------------
echo.
%PY% tools\calibrate_tap_ui.py
echo.
pause
goto MENU
